...
import (
	...
	db "github.com/candacelabs/candace/csf/internal/brainspinedb"
	"github.com/candacelabs/candace/pkg/sqlmigrate"
...
func (store *Postgres) PutDocument(ctx context.Context, document *pb.SourceDocument) (*pb.SourceDocument, error) {
	...
	source, err := queries.CreateSourceRevision(ctx, db.CreateSourceRevisionParams{SourceID: document.SourceId, Revision: document.Revision, ContentHash: document.ContentHash, RawSourceContentHash: nullableText(document.RawSourceContentHash), SourceUri: document.SourceUri, Title: document.Title, MediaType: document.MediaType, License: document.License, RetrievedAt: pgtype.Timestamptz{Time: stamp, Valid: true}})
	if err != nil {
		return nil, err
	}
...
func (store *Postgres) PutNode(ctx context.Context, node *pb.KnowledgeNode) (*pb.KnowledgeNode, error) {
	...
	parameters := db.CreateNodeParams{NodeID: node.NodeId, Kind: strings.ToLower(strings.TrimPrefix(node.Kind.String(), "KNOWLEDGE_KIND_")), SymbolKey: node.SymbolKey, Title: node.Title, Statement: node.Statement, ParentNodeID: nullableText(node.ParentNodeId), AuthorKind: strings.ToLower(strings.TrimPrefix(node.AuthorKind.String(), "AUTHOR_KIND_")), AuthorRef: node.AuthorRef, CitationContentHash: nullableText(node.CitationContentHash), CitationSourceID: nullableText(node.CitationSourceId), CitationRevision: nullableText(node.CitationSourceRevision), CitationChunkID: nullableText(node.CitationChunkId), TextProjectionRef: nullableText(node.TextProjectionRef), VectorProjectionRef: nullableText(node.VectorProjectionRef)}
...
func nodeMessage(node db.BrainspineNode) *pb.KnowledgeNode {
	result := &pb.KnowledgeNode{NodeId: node.NodeID, Kind: pb.KnowledgeKind(pb.KnowledgeKind_value["KNOWLEDGE_KIND_"+strings.ToUpper(node.Kind)]), SymbolKey: node.SymbolKey, Title: node.Title, Statement: node.Statement, ParentNodeId: valueText(node.ParentNodeID), AuthorKind: pb.AuthorKind(pb.AuthorKind_value["AUTHOR_KIND_"+strings.ToUpper(node.AuthorKind)]), AuthorRef: node.AuthorRef, CitationContentHash: valueText(node.CitationContentHash), CitationSourceId: valueText(node.CitationSourceID), CitationSourceRevision: valueText(node.CitationRevision), CitationChunkId: valueText(node.CitationChunkID), TextProjectionRef: valueText(node.TextProjectionRef), VectorProjectionRef: valueText(node.VectorProjectionRef)}
	if node.CitationStartByte != nil {
