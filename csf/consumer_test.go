...
import (
	...
	mocks "github.com/candacelabs/candace/csf/internal/mocks"
...
var _ = Describe("generated transport consumers", func() {
	DescribeTable("distinguishes invalid input from operation failures over HTTP", func(operationError error, status int) {
		api := mocks.NewMockIResearch(gomock.NewController(GinkgoT()))
		api.EXPECT().Search(gomock.Any(), gomock.Any()).Return(nil, operationError)
		engine := httpserver.NewEngine("http-errors")
		csf.NewSurface(api).Register(engine)
	...
	It("carries generated human meanings into the MCP tool listing", func() {
		...
		server, err := surface.MCP.Connect(ctx, serverTransport, nil)
		Expect(err).NotTo(HaveOccurred())
		DeferCleanup(server.Close)
		client := mcp.NewClient(&mcp.Implementation{Name: "human-check", Version: "1"}, nil)
	...
	It("uses the same strict typed operation from the CLI and HTTP consumer", func() {
		...
		csf.NewSurface(service).Register(engine)
	...
	It("preserves the typed HTTP contract through a consumer-owned interface", func() {
		...
		api := mocks.NewMockIResearch(controller)
		api.EXPECT().GetSnapshot(gomock.Any(), gomock.Any()).Return(&pb.GetSnapshotResponse{Snapshot: &pb.Snapshot{Issues: []string{"no observations"}}}, nil)
		surface := csf.NewSurface(api)
		engine := httpserver.NewEngine("test")
		surface.Register(engine)
		server := httptest.NewServer(engine)
		DeferCleanup(server.Close)
		transport := mocks.NewMockIHTTPDoer(controller)
		transport.EXPECT().Do(gomock.Any()).DoAndReturn(func(request *http.Request) (*http.Response, error) { return server.Client().Do(request) })
		client, err := csf.NewClient(server.URL, transport)
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result.Snapshot.Issues).To(Equal([]string{"no observations"}))
	})
	It("lists and executes real MCP tools and rejects unknown fields", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		DeferCleanup(server.Close)
		client := mcp.NewClient(&mcp.Implementation{Name: "consumer", Version: "1"}, nil)
		session, err := client.Connect(ctx, clientTransport, nil)
		Expect(err).NotTo(HaveOccurred())
		DeferCleanup(session.Close)
		listed, err := session.ListTools(ctx, nil)
		Expect(err).NotTo(HaveOccurred())
		contract := reflect.TypeFor[csf.IResearch]()
		names := make([]string, contract.NumMethod())
		for index := range names {
			names[index] = contract.Method(index).Name
		}
