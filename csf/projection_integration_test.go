...
var databaseBudget = patience.Budget{Within: 20 * time.Second}
...
var _ = Describe("PostgreSQL projection integration", func() {
	It("persists queued ingestion and retries through the shared generated HTTP contract", func() {
		const url = "postgres://postgres:projection-test@127.0.0.1:5432/brainspine?sslmode=disable"
		ctx := context.Background()
		pool, err := pgxpool.New(ctx, url)
		Expect(err).NotTo(HaveOccurred())
		DeferCleanup(func() { pool.Close() })
		Expect(patience.Await(GinkgoT(), "disposable PostgreSQL", databaseBudget, func() error { return pool.Ping(ctx) }, func(err error) bool { return err == nil })).To(Succeed())
