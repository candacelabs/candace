...
`github.com/candacelabs/candace/csf`; this release does not introduce a `/v2`
...
[Examples](#examples-and-boundaries) · [Terms](docs/ontology_cgen.md)

## System diagrams

These diagrams are **generated**, not hand-maintained. The shared
[architecture model](docs/architecture.csf) also produces the
[human dictionary](docs/ontology_cgen.md); its syntax is specified in
[EBNF](docs/grammar.ebnf). The OCaml documentation compiler checks identifiers,
...
```mermaid
%% Generated from tools/csf-language/architecture.csf; do not edit.
...
`NewSurface` is the current API's transport-registration helper: it creates no
listener or process. The caller owns the HTTP server and its cancellation.
