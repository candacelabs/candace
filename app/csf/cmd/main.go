...
import (
	...
	"github.com/candacelabs/candace/services/copilot-adapter/copilotbridge"
	...
	"github.com/candacelabs/candace/services/copilot-adapter/workbench"
)
...
func call(arguments []string) error {
	...
	response, err := client.CallOperation(context.Background(), flags.Arg(0), input)
	if err != nil {
		return err
	}
...
func serve(mode string, arguments []string) error {
	...
	if *database != "" {
		...
		options = append(options, csf.WithKnowledge(store, index, artifacts))
	...
	if mode == "mcp" {
		...
		jobs.Go(func() error { defer workerCancel(); return surface.MCP.Run(jobsContext, &mcp.StdioTransport{}) })
		if simulations != nil {
			jobs.Go(func() error { return simulations.Work(jobsContext) })
		}
		return jobs.Wait()
	}
	engine := httpserver.NewEngine("brain-spine", httpserver.WithRequestLogging())
	...
	if *workbenchDatabase != "" {
		...
		bridge, err := copilotbridge.NewCopilotBridge(ctx, copilotbridge.Config{
	...
	engine.Any("/mcp", gin.WrapH(surface.MCPHandler()))
