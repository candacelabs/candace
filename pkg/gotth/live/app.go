...
// state, it must state.
func New[S any](cfg Config[S]) (*App[S], error) {
	...
	behaviour := &appAdapter[S]{cfg: cfg, reg: reg, events: events, comparable: comparableState[S]()}
	app := &App[S]{cfg: cfg, logger: obs.NewLogger(cfg.Logger)}

	app.handler, err = wsx.NewHandler(wsx.Options{
		Origins:                cfg.Origins,
...
func MustNew[S any](cfg Config[S]) *App[S] {
	app, err := New(cfg)
	if err != nil {
		panic(err)
	}
...
func validate[S any](cfg Config[S]) error {
	switch {
	...
	case cfg.Authorize == nil:
		return &ConfigError{Field: "Authorize", Detail: "set a per-event authorization hook, or live.AllowAll to opt out"}
	case cfg.CSRF == nil:
...
func (a *App[S]) Handler() http.Handler { return a.mux }
...
func (a *App[S]) Close(ctx context.Context) error { return a.handler.Close(ctx) }

func (a *App[S]) routes() http.Handler {
...
func (a *App[S]) devOnly(w http.ResponseWriter, what string) bool {
	if a.cfg.Dev {
		return true
	}
...
func (a *appAdapter[S]) Authorize(ctx context.Context, p session.Peer, ev session.Event) error {
...
func (a *appAdapter[S]) Teardown(ctx context.Context, p session.Peer, state any) {
...
func (a *appAdapter[S]) Registry() *render.Registry { return a.reg }

func (a *appAdapter[S]) Registered(name string) bool {
	_, ok := a.events[name]
	return ok
}

func (a *appAdapter[S]) StateComparable() bool { return a.comparable }
...
func sessionOf(p session.Peer) Session {
	return Session{id: ID(p.ID), identity: p.Identity}
}
...
func emissionContext(p session.Peer, scheduledBy uint64) string {
	if scheduledBy == 0 {
		return fmt.Sprintf(
...
func authenticateAdapter(fn func(request *http.Request) (IIdentity, error)) func(request *http.Request) (session.IIdentity, error) {
	return func(r *http.Request) (session.IIdentity, error) {
		...
		if identity == nil {
			return nil, errors.New("gotth-live: the authentication hook returned no identity and no error: return one or the other")
		}
		return identity, nil
	}
}
