...
- `candace/candaceos/` in the private `candace-server` monorepo, which is
  canonical; and
- `candaceos/` in `candacelabs/candace`, the generated public repository that
...
## If this checkout is candacelabs/candace
...
The snapshot is the whole `candace/` tree. Each publish of `candacelabs/candace`
...
- **Every edit is a public release.** This folder sits inside an active
  ...
  published to `candacelabs/candace` and tagged. Run `candace export validate`
...
## What cannot be done from candacelabs/candace alone

The retired `candacelabs/candaceos` snapshot carried this folder without the Go
