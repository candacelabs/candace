...
import (
	...
	"github.com/candacelabs/candace/pkg/cron"
	...
	"github.com/candacelabs/candace/services/copilot-adapter/store"
...
var _ = Describe("the projection of one session's lifecycle events", func() {
	...
	It("keeps an ambiguously delivered prompt unknown until late SDK events durably accept it", func() {
		...
		Expect(items).To(ContainElement(And(
			...
			HaveField("ToolCallID", Equal("late-call")),
		)))
	})
	...
	It("carries the whole transcript item, its call id included, into the transcriptAppended frame", func() {
		...
		frames := patience.Await(GinkgoT(), "two transcriptAppended frames", lifecycleBudget,
			...
			func(rows []storedb.SessionEvent) bool { return len(rows) >= 2 })
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(stored[len(stored)-2].ToolCallID).To(Equal("call-1"))
		Expect(stored[len(stored)-1].ToolCallID).To(Equal("call-1"))
		Expect(stored[len(stored)-1].Kind).To(Equal(string(api.TranscriptItemKindToolResult)))
	})
	...
	It("announces the live subagent aggregate after each activity", func() {
		...
		Expect(version.Summary).To(Equal("current summary"))
